...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/proces'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Proces) {
        return item.id;
    }
    registerChangeInProces() {
        this.eventSubscriber = this.eventManager.subscribe('procesListModification', (response) => this.loadAll());
    }
