...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/usuaris'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Usuaris) {
        return item.id;
    }
    registerChangeInUsuarises() {
        this.eventSubscriber = this.eventManager.subscribe('usuarisListModification', (response) => this.loadAll());
    }
